<template>
  <div class="demo-content">
    ...
    <div class="map-warp" ref="container"></div>
  </div>
</template>
...
import { ScalarFill, WindLayer } from "@sakitam-gis/mapbox-wind";
export default defineComponent({
  setup() {
    ...
    const initMap = () => {
      ...
      map.on("load", async () => {
        const data = await axios.get("https://sakitam.oss-cn-beijing.aliyuncs.com/codepen/wind-layer/json/wind.json").then((res) => {
          return res.data;
        });
        ...
        const fillLayer = new ScalarFill(
          ...
          {
            styleSpec: {
              "fill-color": ["interpolate", ["linear"], ["get", "value"], ...windInterpolateColor],
              opacity: ["interpolate", ["exponential", 0.5], ["zoom"], 5, 1, 8, 1],
            },
            renderForm: "rg",
            widthSegments: 720,
            heightSegments: 360,
